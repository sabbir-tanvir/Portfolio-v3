...
export default function ProjectPage({ params }: { params: { slug: string } }) {
    ...
    const slug = typeof unwrappedParams.slug === 'string' ? unwrappedParams.slug : '';
    
    // Use unwrappedParams.slug instead of params.slug
    const currentProject = projects.find(project => project.slug === unwrappedParams.slug);
    ...
    // Show loading or fallback if project is undefined
    if (!project) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-[#0B0B0D]">
                <p>Loading project...</p>
            </div>
        );
    }
                    ...
                    <div className="container-custom">
                        <div className="pt-28 pb-16">
                            ...
                            <div className="w-full relative rounded-lg overflow-hidden mb-12 bg-gradient-to-r from-indigo-900/40 to-purple-900/40 border border-border">
                                <div className="absolute inset-0 bg-cover bg-center opacity-20" style={{ backgroundImage: `url('${project.previewImages[0]}')` }}></div>
                            ...
                            <section className="mb-16">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                    {project.previewImages.map((image, index) => (
                                        <div key={index} className="relative rounded-xl overflow-hidden border border-border group transition-all duration-300 hover:shadow-xl">
                                            <Image
                                                src={image}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {project.implementation.frontend && (
                                        <div className="bg-card/50 backdrop-blur-sm p-8 rounded-lg border border-border hover:border-primary/50 transition-colors">
                                            ...
                                            <ul className="space-y-2 text-foreground/70">
                                                {project.implementation.frontend.map((item, index) => (
                                    ...
                                    {project.implementation.backend && (
                                        <div className="bg-card/50 backdrop-blur-sm p-8 rounded-lg border border-border hover:border-primary/50 transition-colors">
                                            ...
                                            <ul className="space-y-2 text-foreground/70">
                                                {project.implementation.backend.map((item, index) => (
                                                    <li key={index} className="flex items-start">
                                                        <svg className="h-5 w-5 text-primary mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                                                        </svg>
                                    ...
                                    {project.implementation.deployment && (
                                        <div className="bg-card/50 backdrop-blur-sm p-8 rounded-lg border border-border hover:border-primary/50 transition-colors">
                                            ...
                                            <ul className="space-y-2 text-foreground/70">
                                                {project.implementation.deployment.map((item, index) => (
                                                    <li key={index} className="flex items-start">
                                                        <svg className="h-5 w-5 text-primary mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                                                        </svg>
                            ...
                            <section className="mb-16">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {project.features.map((feature, index) => (
                                        <div key={index} className="bg-card/50 backdrop-blur-sm p-6 rounded-lg border border-border hover:border-primary/50 transition-colors">
